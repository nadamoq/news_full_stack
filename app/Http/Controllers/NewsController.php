...
class NewsController extends Controller
{
    ...
    public function publish ($id){
        ...
        return response()->json(['message'=>$result?'successfully published':'error'],$result?200:Response::HTTP_BAD_REQUEST);
    }
    public function archive($id){
        ...
        if($news->status=='archived'){
            ...
            return response()->json(['message'=>$result?'unarchived':'error','button'=>'archive'],$result?200:Response::HTTP_BAD_REQUEST);
        }
        elseif($news->status=='published'){
            $result=$news->update(['status'=>'archived']);
            return response()->json(['message'=>$result?'archived':'error','button'=>'unarchive'],$result?200:Response::HTTP_BAD_REQUEST);
        }else{
